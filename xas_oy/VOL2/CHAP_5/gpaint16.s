...
gpaint_b16:
	...
	move.w	BCOL(a1),d6	*d6 = 境界色

	move.w	COL(a1),d7	*d7 = 描画色
	swap.w	d7		*
	move.w	COL(a1),d7	*
	bset.l	d7,d6		*描画色はつねに境界色
...
spix2:
sloop1:	move.w	(a0)+,d5	*境界色部分を飛ばす
	btst.l	d5,d6		*
	dbeq	d3,sloop1	*
	bne	sdone		*

	move.l	a0,d5
	subq.l	#2,d5		*d5 = 非境界色部分左端アドレス
...
sloop2:	move.w	(a0)+,d5	*非境界色部分を飛ばす
	btst.l	d5,d6		*
	dbne	d3,sloop2	*
	beq	bound		*
	subq.l	#2,a0		*
