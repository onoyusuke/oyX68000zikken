...
expand_comp2:
	...
	move.l	pset(pc),d5	*d5.l = １ピクセル書き込んでから
				*　参照位置を移動する命令コード
...
Explp2:	add.w	(a4)+,d4	*
	sub.w	d2,d1		*
	bgt	Explp2		*
Expnxt:	move.l	d5,(a5)+	*move,leaの２命令を書き込む
	move.w	d4,(a5)+	*アドレスの変位を書き込む
				*（leaのディスプレースメント）
Expskp:	dbra	d3,Explp1
	move.w	term(pc),-4(a5)	*rts命令を書き込む
	FLUSH_CACHE
term:	rts

pset:	move.w	(a4),(a0)+	*１ピクセル書き込む命令
