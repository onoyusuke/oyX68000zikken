*	グラフィックパターンを拡大/縮小してプットする（拡大時処理高速化版）
...
fix1:	add.w	d3,d5		*d5 = yループカウンタ初期値
	...
	move.w	a3,d7		*d7 = yについての誤差項補正値

	move.w	d4,d0
	addq.w	#1,d0
	lea.l	cnext(pc),a2	*a2 = 戻りアドレス
	bclr.l	#0,d0		*横ピクセル数は奇数か？
	beq	skip		*
	subq.l	#2,a2
skip:	lea.l	alincopy(pc),a3
	suba.w	d0,a3
...
cnext:	add.w	d7,d1
	bgt	yinclp
lincpy:	dbra	d5,cpylp1
	bra	done

	.dcb.w	GNPIXEL/2,$2adc	*move.l (a4)+,(a5)+
alincopy:
	jmp	(a2)
