*	グラフィックパターンを拡大/縮小してプットする（透明色対応版）
...
yloop:	movea.l	a0,a5		*a5 = 描画先
	...
	movea.l	d2,a4		*a4 = テーブル先頭アドレス
	move.w	d4,d0		*d0 = xループカウンタ
xloop:	move.w	(a6),d7		*d7=描画色
	beq	xskip		*透明色部分は飛ばす
	move.w	d7,(a5)+	*１ピクセル書き込む
	adda.w	(a4)+,a6	*参照元y座標を進める
	dbra	d0,xloop	*横幅分繰り返す
	bra	xdone

xskip:	addq.l	#2,a5		*描画先x座標を進める
	adda.w	(a4)+,a6	*参照元x座標を進める
	dbra	d0,xloop	*横幅分繰り返す

xdone:	add.w	a3,d1		*yについての誤差を累積させる
	ble	ynext		*
