...
*
doit:
	move.w	#WOPEN,-(sp)	*更新モードで
	pea.l	arg		*　ファイルをオープンする
	DOS	_OPEN		*
	move.w	d0,d1		*d1=ファイルハンドル
	bmi	werror		*d1が負ならエラー
				*（多分、書き込み禁止）

	DOS	_PRINT		*ついでにファイル名を表示
	addq.l	#6,sp
	...
	move.w	filbuf+24,d0	*d0の下位ワード＝変更最終年月日
	swap.w	d0		*d0の上位ワード＝変更最終年月日
	
	move.w	#$6000,d0	*d0の下位ワード=12:00:00
				*$6000 = %01100_000000_00000
				*	  HHHHH MMMMMM SSSSS
...
*
	...
	.even
*
usgmes:	.dc.b	'機　能：ファイルの最終更新時刻を12:00:00に変更します',CR,LF
	.dc.b	TAB,'ファイル名にはワイルドカードが使用できます',CR,LF
	.dc.b	'使用法：FNOON［ファイル名］'
crlfms:	.dc.b	CR,LF,0
...
errmes:	.dc.b	'FNOON：書き込み禁止ファイルです（たぶん）',CR,LF,0
