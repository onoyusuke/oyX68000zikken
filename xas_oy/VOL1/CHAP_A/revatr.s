...
*
doit:
	moveq.l	#0,d0		*一応上位バイトをクリア
	move.b	filbuf+21,d0	*d0.w＝ファイル属性
*eor.b	#READONLY,d0
	eor.b	#HIDDEN,d0	*不可視属性を反転する
	move.w	d0,-(sp)	*属性
	pea.l	arg		*ファイル名
	DOS	_CHMOD		*属性変更
		*本当はここでエラーチェックをすべきだが省略する
	DOS	_PRINT		*ついでにファイル名表示
	addq.l	#6,sp		*
...
*
	...
	.even
*
usgmes:	.dc.b	'機　能：ファイルの不可視属性を反転します',CR,LF
	.dc.b	TAB,'ファイル名にはワイルドカードが使用できます',CR,LF
	.dc.b	'使用法：REVATR［ファイル名］'
crlfms:	.dc.b	CR,LF,0
